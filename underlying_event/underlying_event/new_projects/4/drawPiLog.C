...
void drawPiLog(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0,0.45);
...
label->DrawLatexNDC(0.03, 0.5, "(p+#bar{p})/(#pi^{+}+#pi^{-})"); // 使用NDC坐标
  ...
  std::vector<TH1D*> projections4 = getCentPtProjections(filename, histname4);
