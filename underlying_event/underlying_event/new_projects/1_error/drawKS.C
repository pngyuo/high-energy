...
void drawKS(){
  ...
  c1->cd(3)->SetLeftMargin(0);
    ...
    TString histname[3] = {"pKSAvgPt_NT0_ptcut", "pKSAvgPt_NT2_ptcut", "pKSAvgPt_NT1_ptcut"};
    ...
    for (int j = 0; j < 3; j++) {
        ...
        TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0.5, 2);
  ...
  TGraph* graph = drawGraphs(gPad, projections, color, markerStyle, 2,averageNT); 
    ...
    c1->SaveAs("KS.A.all.png");
    c1->SaveAs("KS.A.all.pdf");
}
