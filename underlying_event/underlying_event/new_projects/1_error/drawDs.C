...
void drawDs(){
  ...
  c1->cd(3)->SetLeftMargin(0);
    ...
    TString histname[3] = {"pDsAvgPt_NT0_ptcut", "pDsAvgPt_NT2_ptcut", "pDsAvgPt_NT1_ptcut"};
    ...
    for (int j = 0; j < 3; j++) {
        ...
        TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0.5,5);
  ...
  TGraph* graph = drawGraphs(gPad, projections, color, markerStyle, 2,averageNT); 
    ...
    c1->SaveAs("Ds.P.all.png");
    c1->SaveAs("Ds.P.all.pdf");
}
