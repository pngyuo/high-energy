...
void drawP(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, -0.03, 0.7);
...
label->DrawLatexNDC(0.03, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
  ...
  drawGraphs(gPad,projections1, projections2,colors, markerStyles);
    ...
    TLegend *legend = new TLegend(0.15, 0.72, 0.35,0.84);
  ...
  addText4(0.63, 0.20, "p+#bar{p}");
   //c1->SetLogy(); // 设置y轴为对数刻度
  c1->SaveAs("proton_Monash.png");
