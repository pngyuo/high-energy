...
double rtRanges[3] = {0, 0.5, 2};  // RT 的范围
    double rtUpperRanges[3] = {0.5, 2, 30 / averageNT};  // RT 上限
...
void drawGraphs(TVirtualPad *pad, const std::vector<TH1D*>& projections1,const std::vector<Color_t>& colors, const std::vector<Style_t>& markerStyles) {
  pad->cd(); // 切换到当前子画布
  ...
  TGraph* graph = drawConnectedPoints(hist1, color, markerStyle);
    if(i == 0) {
...
label->DrawLatexNDC(0.02, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
...
TString filenames = "hist_outputnohFSI.root";
  TString histname1[3] = {"hProton_dPhi0", "hProton_dPhi2", "hProton_dPhi1"};
    ...
    gStyle->SetOptStat(kFALSE);
     for (int j = 0; j < 3; j++) {
    c1->cd(j+1);
   TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 1e-3,0.6);
    axisFrame->Draw("axis");  // 只绘制坐标轴，不绘制直方图
          std::vector<TH1D*> projections1 = getCentPtProjections(filenames, histname1[j]); // 获取投影
      drawGraphs(gPad,projections1, colors, markerStyles); // 分析并绘制每个投影
    ...
    if (j == 0) {
        ...
        addText2(0.4, 0.8, "Away  nohFSI");
        addText4(0.36, 0.25, "p+#bar{p}");
    } else if (j == 2) {
    ...
    c1->cd(1);
    TLegend *legend = new TLegend(0.72, 0.65, 0.97, 0.86);
