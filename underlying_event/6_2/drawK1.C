...
void drawK1() {
    ...
    TLatex *label = new TLatex(0.01, 0.6, "(p+#bar{p})/(#pi^{+}+#pi^{-})");
    ...
    TString histname[3] = {"hProton_dPhi0", "hProton_dPhi2", "hProton_dPhi1"};
    TString histname1[3] = {"hPiCh_dPhi0", "hPiCh_dPhi2", "hPiCh_dPhi1"};
    TString filenames1[3]= {"p_pi_to.root", "p_pi_aw.root", "p_pi_tr.root"};
    ...
    TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5, 100, 0,0.2);
    axisFrame->Draw("axis");  // 只绘制坐标轴，不绘制直方图

        for (int i = 0; i < 3; i++) {
            Color_t color = colors[i];
            Style_t markerStyle = markerStyles[i];
            TH1D* projections = getCentPtProjections(filenames[i], histname[j], averageNT, rebinFactor); // 获取投影并rebin
            TH1D* projections1 = getCentPtProjections(filenames[i], histname1[j], averageNT, rebinFactor); // 获取投影并rebin
