...
std::vector<TH1D*> getCentPtProjections(const TString& filename,double& averageNT) {
    ...
    projections.push_back(chargedParticlesCount);
    return projections;
}
...
void drawK(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 35.1, 100, 1e-5, 0.2);
