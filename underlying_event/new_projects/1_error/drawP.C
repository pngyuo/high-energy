...
void drawP(){
  ...
  c1->cd(3)->SetLeftMargin(0);
    ...
    TString histname[3] = {"pPAvgPt_NT0_ptcut", "pPAvgPt_NT2_ptcut", "pPAvgPt_NT1_ptcut"};
    TString filenames1[3]= {"proton_toward.root", "proton_away.root", "proton_transverse.root"};
    ...
    for (int j = 0; j < 3; j++) {
        ...
        TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0.6, 2.2);
