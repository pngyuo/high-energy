      ...
      //double dEtaAbs = TMath::Abs(dEta); 
	    ...
	    int dPhi_segment = 0;
		  if (dPhi >= 0 && dPhi < TMath::Pi() / 3) {
	        ...
	        if(abs(id)==321) {
           	if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pKAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hKCh_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pKAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hKCh_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pKAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hKCh_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            }
	        }
	        if(abs(id)==211) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pPiAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPiAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hPiCh_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pPiAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPiAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hPiCh_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pPiAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPiAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hPiCh_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
					...
					if(abs(id)==2212) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pPAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hProton_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pPAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hProton_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pPAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pPAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hProton_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
					...
					if(abs(id)==431) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pDsAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pDsAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hDs_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pDsAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pDsAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hDs_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pDsAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pDsAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hDs_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            }
					}
					if(abs(id)==4122) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pLcAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLcAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hLambc_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pLcAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLcAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hLambc_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pLcAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLcAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
    	...
    	if(fabs(dRap)<0.5){
    		  if(abs(id)==3122) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pLAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hLambda_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pLAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hLambda_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pLAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pLAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hLambda_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            }
					}
					if(abs(id)==411) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pD0AvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pD0AvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hD0_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pD0AvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pD0AvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hD0_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pD0AvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pD0AvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
              hD0_dPhi2->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            }
					}
					if(abs(id)==310) {
            if (dPhi_segment == 0) {
              if(vec_tmp.Pt()<5) pKSAvgPt_NT0_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKSAvgPt_NT0->Fill(chargedParticlesCount, vec_tmp.Pt());
            	hKshort_dPhi0->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 1) {
              if(vec_tmp.Pt()<5) pKSAvgPt_NT1_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKSAvgPt_NT1->Fill(chargedParticlesCount, vec_tmp.Pt());
              hKshort_dPhi1->Fill(vec_tmp.Pt(), dNtrkV0, chargedParticlesCount);
            } else if (dPhi_segment == 2) {
              if(vec_tmp.Pt()<5) pKSAvgPt_NT2_ptcut->Fill(chargedParticlesCount, vec_tmp.Pt());
              pKSAvgPt_NT2->Fill(chargedParticlesCount, vec_tmp.Pt());
