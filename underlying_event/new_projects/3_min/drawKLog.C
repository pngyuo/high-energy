...
std::vector<TH1D*> getCentPtProjections(const TString& filename, const TString& histname) {
  ...
  double rtRanges[3] = {0, 0.5,2};  // RT 的范围
    double rtUpperRanges[3] = {0.5, 2, 5};  // RT 上限
...
void drawKLog(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, -0.7, 0.4);
...
label->DrawLatexNDC(0.03, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
  ...
  std::vector<TH1D*> projections2 = getCentPtProjections1(filename, histname2);
