...
std::vector<TH1D*> getCentPtProjections(const TString& filename, const TString& histname) {
  ...
  double rtRanges[4] = {0, 0.5, 1.5, 2.5};  // RT 的范围
    double rtUpperRanges[4] = {0.5, 1.5, 2.5, 5};  // RT 上限
...
void drawK(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, -0.01, 0.9);
...
label->DrawLatexNDC(0.03, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
  ...
  // 定义标记样式数组
  std::vector<Style_t> markerStyles = {24, 20, 21, 22};
  ...
  drawGraphs(gPad,projections1, projections2,colors, markerStyles);
    ...
    TLegend *legend = new TLegend(0.15, 0.12, 0.29,0.3);
    ...
    legend->AddEntry(marker3, "1.5<R_{T}<2.5", "lp");
  ...
  addText4(0.53, 0.20, "#Kappa^{+}+#Kappa^{-}");
