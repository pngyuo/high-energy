...
void drawKLog(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, -0.5,0.6);
...
label->DrawLatexNDC(0.03, 0.5, "(#Kappa^{+}+#Kappa^{-})/(#pi^{+}+#pi^{-})"); // 使用NDC坐标
  ...
  TString filename = "hist_outputallFSI.root"; 
  ...
  std::vector<TH1D*> projections4 = getCentPtProjections1(filename, histname4);
