...
void get_ppPpi_Data(){
	...
	TFile *input = new TFile("HEPData-ins2626034-v1-Table_100.root");//修改一：改为下载的名字
  //output file name convention: system_energy_particleSpecies.root
	TFile *output = new TFile("p_pi_ratio_toward_05.root","recreate");//修改二：把ppi_Tr_25_5.root改为你想要输出的名字，最好与提取的文件数据相关，避免之后混淆

	TDirectoryFile *file;


	file = (TDirectoryFile*)input->Get("Table 100");//修改三：提取的数据是table什么就改为什么，有数字也有字母的
