...
C            write(14,211) ITYP(I), PX(I), PY(I), PZ(I), XMASS(I),
C      1          GX(I), GY(I), GZ(I), FT(I)
C c
C  1015 CONTINUE
C  211  format(I6,2(1x,f8.3),1x,f10.3,1x,f6.3,4(1x,e8.2))
...
c     such that ARRIN(INDEX(J)) is in ascending order for J=1,...,m
 ...
 10   continue
      if (l .gt. 1) then
         ...
         indx(ir) = indx(1)
         ir = ir - 1
         !liang-12/2020 liang changed to cope with m=1 case
         !if (ir .eq. 1) then
         if (ir .eq. 0 .or. ir .eq. 1) then
            indx(1) = indxt
            return
         end if
      end if
