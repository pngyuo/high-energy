...
clin-2/2017 this may cause no particle record in ampt.dat for the event:
         if(FT0(I).le.0) write(6,*) 'ERROR:FT0<=0',I,FT0(I),ITYP0(I),
...
c... Di-Omega
clin-8/2018 Change Di-Omega IPDG to 3333:
c      IF (IPDG .EQ. 6666) THEN
      IF (IPDG .EQ. 3333) THEN
         IARFLV = 44
         RETURN
      END IF
c sp06/05/01 end    
...
clin-8/2018 keep the same ID for heavy flavors including quarkonia:
      IF ((iabs(IPDG).gt.400.and.iabs(IPDG).lt.700).or.
     1     (iabs(IPDG).gt.4000.and.iabs(IPDG).lt.7000)
     2     .or.(IPDG.gt.10000.and.MOD(IPDG/100,10).ge.4
     3     .and.MOD(IPDG/100,10).le.6)) THEN
         IARFLV = IPDG
         RETURN
      END IF
...
clin-8/2018 keep the same ID for heavy flavors including quarkonia:
      IF ((iabs(IART).gt.400.and.iabs(IART).lt.700).or.
     1     (iabs(IART).gt.4000.and.iabs(IART).lt.7000)
     2     .or.(IART.gt.10000.and.MOD(IART/100,10).ge.4
     3     .and.MOD(IART/100,10).le.6)) THEN
         INVFLV = IART
         RETURN
      END IF
c
...
c            Ieta = 1 + int(ABS(eta) / BY)
            ...
            END IF
clin-8/2018 some new charmonia states have ITYP > 10000:
c            IF (ITYP .GT. 1000) THEN
            IF (ITYP .GT. 1000 .and. ITYP .LT. 10000) THEN
c
               dy1ntb(IY) = dy1ntb(IY) + 1.0
            END IF
