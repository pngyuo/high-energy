...
void SaveSumRidgePlots(TH1D *hSumRidge, TString name) {
  ...
  void calc_flow_fn_longRange() {
    TFile *file_1 = new TFile("hist_ampt_3q_0.15mb_longRange_yuhao.root");
    TFile *file_2 = new TFile("hist_ampt_3q_0.15mb_longRange_yuhao.root");
    ...
    double c2_Eta[5];
    double c2_Eta_err[5];
    double a2_Eta_high[5];
    double a2_Eta_err[5];
    double a3_Eta_high[5];      // 存储a3原始值
    double a3_Eta_err[5];       // 存储a3误差
    double c3_Eta[5];           // 存储修正后的c3值
    double c3_Eta_err[5];       // 存储c3误差
    ...
    TH3D *hsame_centlow = (TH3D *)file_1->Get("hDEtaDPhiTrigEtaSameEvent_CentFMD12FMD30");
    TH3D *hmixed_centlow = (TH3D *)file_1->Get("hDEtaDPhiTrigEtaMixEvent_CentFMD12FMD30");
    ...
    for (int icent = 1; icent < N_cent; icent++) {
        ...
        if (Nsel_val[icent] < 85) {
            if (icent == 0) {
                hTrigPt_cent[icent] = (TH1D *)file_1->Get(Form("hTrigPt_CentFMD12FMD3%d", icent));
            } else {
                hsame_cent[icent] = (TH3D *)file_1->Get(Form("hDEtaDPhiTrigEtaSameEvent_CentFMD12FMD3%d", icent));
                hmixed_cent[icent] = (TH3D *)file_1->Get(Form("hDEtaDPhiTrigEtaMixEvent_CentFMD12FMD3%d", icent));
                hTrigPt_cent[icent] = (TH1D *)file_1->Get(Form("hTrigPt_CentFMD12FMD3%d", icent));
            }
        } else {
            if (icent == 0) {
                hTrigPt_cent[icent] = (TH1D *)file_2->Get(Form("hTrigPt_CentFMD12FMD3%d", icent));
            } else {
                hsame_cent[icent] = (TH3D *)file_2->Get(Form("hDEtaDPhiTrigEtaSameEvent_CentFMD12FMD3%d", icent));
                hmixed_cent[icent] = (TH3D *)file_2->Get(Form("hDEtaDPhiTrigEtaMixEvent_CentFMD12FMD3%d", icent));
                hTrigPt_cent[icent] = (TH1D *)file_2->Get(Form("hTrigPt_CentFMD12FMD3%d", icent));
            }
        }
        ...
        for (int iEta = 0; iEta < nEtaBins; iEta++) {
            ...
            etaValues[iEta] = eta;
            // 计算对应的Z bin（从6开始）
            int zBin = iEta + 56; // TH3D的Z bin索引从1开始，有效数据在6到55
            ...
            if (hsame_eta_high->GetEntries() > 0 && hmix_eta_high->GetEntries() > 0) {
                hYHigh = getYFromHist(hsame_eta_high, hmix_eta_high, Form("YHigh_etaFMD12FMD3%d", iEta), 100);
                hYLow = getYFromHist(hsame_eta_low, hmix_eta_low, Form("YLow_etaFMD12FMD3%d", iEta), 100);
                double a2higherr = 0;
                double a2lowerr = 0;
    ...
    gr_f2_raw->GetYaxis()->SetRangeUser(0.006, 1.2);
