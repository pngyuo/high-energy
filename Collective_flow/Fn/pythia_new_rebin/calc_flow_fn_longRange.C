...
void fitAndSave(TGraphErrors* graph, const TString& name, double& F_n, double& F_n_err) {
    ...
    fitFunc->SetParameters(0.1, 0.03, 0.03); // 初始参数
    graph->Fit("fitFunc", "Q");
...
double getA3(TH1D *hY, double &a1, double &a2, double &a3, double &a4, double &G, double &chi2, int &ndf, double &a3err) {
    ...
    a3 = fitFun->GetParameter(3);
    a4 = fitFun->GetParameter(4);
    a3err=fitFun->GetParError(3);  // 修正为获取a3的误差
    G = gYIntegral / (2.0 * PI); // 计算最终的G值
    chi2 = fitFun->GetChisquare();
    ndf = fitFun->GetNDF();
    SavePlot(hY, hY->GetName());
...
// 新增：rebin数据函数
void rebinData(int nPoints, double* x, double* y, double* yErr, double* xWidth,
              int rebinFactor, int& nRebinned, double* xRebinned, double* yRebinned, 
              double* yErrRebinned, double* xWidthRebinned) {
    nRebinned = nPoints / rebinFactor;
    double binWidth = xWidth[0]; // 假设所有原始bin宽度相同

    for (int i = 0; i < nRebinned; i++) {
        double sumY = 0.0;
        double sumYErrSq = 0.0;
        double sumX = 0.0;
        
        int startIdx = i * rebinFactor;
        int endIdx = startIdx + rebinFactor;
        
        for (int j = startIdx; j < endIdx; j++) {
            sumY += y[j];
            sumYErrSq += yErr[j] * yErr[j];
            sumX += x[j];
        }
        
        // 计算平均值
        yRebinned[i] = sumY / rebinFactor;
        yErrRebinned[i] = sqrt(sumYErrSq) / rebinFactor;
        xRebinned[i] = sumX / rebinFactor;
        xWidthRebinned[i] = rebinFactor * binWidth;
    }
}
...
void calc_flow_fn_longRange() {
    ...
    // 新增：rebin相关参数
    const int rebinFactor = 5; // 每5个原始点合并为1个
    int nRebinned = nEtaBins / rebinFactor;
    double a2_rebin[nRebinned], a2_err_rebin[nRebinned];
    double a3_rebin[nRebinned], a3_err_rebin[nRebinned];
    double c2_rebin[nRebinned], c2_err_rebin[nRebinned];
    double c3_rebin[nRebinned], c3_err_rebin[nRebinned];
    double eta_rebin[nRebinned], etaWidth_rebin[nRebinned];
    ...
    for (int icent = 1; icent < N_cent; icent++) {
        ...
        for (int iEta = 0; iEta < nEtaBins; iEta++) {
            ...
            if (hsame_eta_high->GetEntries() > 0 && hmix_eta_high->GetEntries() > 0) {
                ...
                cout << "c2" << c2 << endl;
                c2_Eta[iEta] = c2;
                double c2_err = sqrt(pow(a2higherr, 2) + pow(a2lowerr * k, 2)); // 修正误差计算
                ...
                cout << "c3" << c3 << endl;
                c3_Eta[iEta] = c3;
                double c3_err = sqrt(pow(a3higherr, 2) + pow(a3lowerr * k, 2)); // 修正误差计算
        ...
        // 新增：对数据进行rebin
        rebinData(nEtaBins, etaValues, a2_Eta_high, a2_Eta_err, etaBinWidths, 
                  rebinFactor, nRebinned, eta_rebin, a2_rebin, a2_err_rebin, etaWidth_rebin);
        
        rebinData(nEtaBins, etaValues, a3_Eta_high, a3_Eta_err, etaBinWidths, 
                  rebinFactor, nRebinned, eta_rebin, a3_rebin, a3_err_rebin, etaWidth_rebin);
        
        rebinData(nEtaBins, etaValues, c2_Eta, c2_Eta_err, etaBinWidths, 
                  rebinFactor, nRebinned, eta_rebin, c2_rebin, c2_err_rebin, etaWidth_rebin);
        
        rebinData(nEtaBins, etaValues, c3_Eta, c3_Eta_err, etaBinWidths, 
                  rebinFactor, nRebinned, eta_rebin, c3_rebin, c3_err_rebin, etaWidth_rebin);

        // 使用rebin后的数据创建图表
        TGraphErrors *gr_an_f2 = new TGraphErrors(nRebinned, eta_rebin, a2_rebin, etaWidth_rebin, a2_err_rebin);
        TGraphErrors *gr_an_f3 = new TGraphErrors(nRebinned, eta_rebin, a3_rebin, etaWidth_rebin, a3_err_rebin);
        ...
        // 使用rebin后的数据创建图表
        TGraphErrors *gr_cn_f2 = new TGraphErrors(nRebinned, eta_rebin, c2_rebin, etaWidth_rebin, c2_err_rebin);
        TGraphErrors *gr_cn_f3 = new TGraphErrors(nRebinned, eta_rebin, c3_rebin, etaWidth_rebin, c3_err_rebin);
    ...
    TGraphErrors *gr_f3_raw = new TGraphErrors(N_cent, Nsel_val, cent_an_f3, Nsel_bin_width, err_an_f3);
    TGraphErrors *gr_f3_sub = new TGraphErrors(N_cent, Nsel_val, cent_cn_f3, Nsel_bin_width, err_cn_f3);
    ...
    gr_f2_raw->GetYaxis()->SetRangeUser(-0.1, 0.1);
    ...
    cCombined->SaveAs("Fn_vs_Nch_rebin.png");
