...
int main(int argc, char** argv)
{
  ...
  TH1D *hRefPtCountlow = new TH1D("hRefPtCountlow", ";N_{ref}; counts", 2000, 0, 4000);
  TH1D *hRefPtCounthigh = new TH1D("hRefPtCounthigh", ";N_{ref}; counts", 2000, 0, 4000);
  ...
  for(int i=0; i<N_cent; i++) {
    ...
    hRefPtCount[i] = new TH1D(Form("hRefPtCount%d",i), ";N_{ref}; counts", 2000, 0, 4000);
...
//  if(nEvents>8e6) nEvents=8e6;
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
    ...
    int dMidCh=0;
    int nRefParticleslow = 0; 
    int nRefParticleshigh = 0; 
    int nRefParticles = 0;
    ...
    for (int iTrack = 0; iTrack < nTracks; ++iTrack) {
      ...
      if(pass_id&&pt>0.4&&fabs(vec_tmp.Eta())<2.5) dMidCh++;//Nsel
...
//			hSphericity->Fill(dMidCh, dSph);
    ...
    if (dMidCh>=10 && dMidCh<30){
    nRefParticleslow = nRefParticles;
    hRefPtCountlow->Fill(nRefParticleslow);
  }

    if (dMidCh>=60){
    nRefParticleshigh = nRefParticles;
    hRefPtCounthigh->Fill(nRefParticleshigh);
  }
    ...
    if (icent >= 0 && icent < N_cent) {
    hRefPtCount[icent]->Fill(nRefParticles); // 使用事件级计数器
    }
  ...
  f->cd();
	...
	hNChMid->Write();
  hRefPtCountlow->Write();
  hRefPtCounthigh->Write();
