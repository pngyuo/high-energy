...
void calc_flow_ancn_longRange_rebin() {
    ...
    for (int iEta = 0; iEta < nEtaBins; iEta++) {
        ...
        etaValues[iEta] = eta;
        int zBinmin = 2*iEta + 6; // TH3D的Z bin索引从1开始，有效数据在6到55
        int zBinmax = 2*iEta + 7; // TH3D的Z bin索引从1开始，有效数据在6到55
        ...
        TH2D *hTrigPtEtaLow = (TH2D *)file_1->Get("hTrigPtEtaLow");
        TH2D *hTrigPtEtaHigh = (TH2D *)file_1->Get("hTrigPtEtaHigh");
        ...
        if (hsame_eta_high->GetEntries() > 0 && hmix_eta_high->GetEntries() > 0) {
            hYHigh = getYFromHist(hsame_eta_high, hmix_eta_high, Form("YHigh_eta%d", iEta),10);
            hYLow = getYFromHist(hsame_eta_low, hmix_eta_low, Form("YLow_eta%d", iEta),10);
    ...
    // 创建图形时使用半宽作为横坐标误差
    TGraphErrors *gr_c2_rebinned = new TGraphErrors(nEtaBins, etaValues, c2_Eta, etaBinHalfWidths, c2_Eta_err);
    TGraphErrors *gr_a2_high_rebinned = new TGraphErrors(nEtaBins, etaValues, a2_Eta_high, etaBinHalfWidths, a2_Eta_err_high);
    TGraphErrors *gr_a2_low_rebinned = new TGraphErrors(nEtaBins, etaValues, a2_Eta_low, etaBinHalfWidths, a2_Eta_err_low);
    ...
    gr_c2_rebinned->GetYaxis()->SetRangeUser(0.005, 0.012);
