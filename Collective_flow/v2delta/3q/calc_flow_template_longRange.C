...
double funTemplate(double *x, double *par){
  ...
  double v3 = par[2];
  //double G = par[4];
  TF1 *funRidge = new TF1("funRidge", "[0]*(1+2*[1]*cos(2*x)+2*[2]*cos(3*x))", -PI*0.5, PI*1.5);
...
void calc_flow_template_longRange_new(){
  ...
  if (v22FMD12FMD3 != 0) {
	...
	TFile *infile_Pid_Low = new TFile("hist_ampt_3q_1.5mb_longRange_yuhao.root");
	TFile *infile_Pid_High = new TFile("hist_ampt_3q_1.5mb_longRange_yuhao.root");
