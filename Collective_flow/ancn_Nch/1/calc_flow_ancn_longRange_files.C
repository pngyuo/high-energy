                 ...
                 )
{
    ...
    TFile *file_2 = new TFile(filename);   // 两个句柄指向同一文件，保持原来逻辑
    ...
    TH1D *hYLow = getY(hSameLowRidge, hMixLowRidge,
                       "YLow", hTrigPtLow_1->GetEntries());
    hYLow->Rebin(2);

    TH1D *hSameHighRidge = sumRidge(hsameHigh_1, "sameHigh");
    TH1D *hMixHighRidge  = sumRidge(hmixedHigh_1, "mixHigh");
    TH1D *hYHigh = getY(hSameHighRidge, hMixHighRidge,
                        "YHigh", hTrigPtHigh_1->GetEntries());

    //-------------------------------------------------------
    // 2. 0.3–3 GeV/c 粒子 a2
    //-------------------------------------------------------
    double a1_0 = 0, a2_0 = 0, a3_0 = 0, a4_0 = 0;
    double G_0  = 0, chi2_0 = 0;
    int    ndf_0 = 0;
    double a2err_0 = 0;

    // 注意：这里用 hYHigh 做拟合，hYLow 仅作为占位（原代码逻辑）
    double a2 = getAn(hYHigh,
                      a1_0, a2_0, a3_0, a4_0,
                      G_0, chi2_0, ndf_0, a2err_0,
                      "HighPt", label, "all");

    cout << label << "  a2 = " << a2 << " ± " << a2err_0 << endl;
