...
TGraph* drawConnectedPoints(TH1D* hist, Color_t color, Style_t markerStyle, double& averageNT, int rebinFactor = 1) {
...
std::vector<TH1D*> getCentPtProjections(const TString& filename, const TString& histname, double& averageNT, int rebinFactor = 1) {
...
void drawGraphs(TVirtualPad* pad, const std::vector<TH1D*>& projections1, std::vector<TH1D*> projections2, std::vector<TH1D*> projections3, std::vector<TH1D*> projections4, Color_t color, Style_t markerStyle, double& averageNT, int rebinFactor) {
    pad->cd(); // 切换到当前子画布
    for (size_t i = 0; i < projections1.size(); ++i) {
        ...
        diffHist2->Add(hist4, -1); // 减去hist4
        // 相除操作：hist3 / hist4
        TH1D* ratioHist = (TH1D*)diffHist1->Clone();
        ratioHist->Divide(diffHist2); // 除以hist4

        // 绘制相减和相除的结果
        TGraph* graphRatio = drawConnectedPoints(ratioHist, color, markerStyle, averageNT, rebinFactor);
...
void drawGraphs1(TVirtualPad *pad, const std::vector<TH1D*>& projections1, std::vector<TH1D*> projections2, std::vector<TH1D*> projections3, std::vector<TH1D*> projections4, Color_t color, Style_t markerStyle,double& averageNT,int rebinFactor) {
  pad->cd(); // 切换到当前子画布
  for (size_t i = 0; i < projections1.size(); ++i) {
    TH1D* hist1 = projections1[i];
    TH1D* hist2 = projections2[i];
 ...
 TH1D* ratio = (TH1D*)ratioHist->Clone();
    ...
    // 绘制相减和相除的结果
    TGraph* graphRatio = drawConnectedPoints(ratio, color, markerStyle,averageNT,rebinFactor);
...
void drawPi() {
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0,0.2);
  ...
  axisFrame->Draw("axis");  // 只绘制坐标轴，不绘制直方图
    ...
    int rebinFactor = 3; // 设置rebin因子
    for (int i = 0; i < 3; ++i) {
        Color_t color = colors[i];
        Style_t markerStyle = markerStyles[i];
        std::vector<TH1D*> projections1 = getCentPtProjections(filenames[i], histname1, averageNT, rebinFactor);
        std::vector<TH1D*> projections2 = getCentPtProjections(filenames[i], histname2, averageNT, rebinFactor);
        std::vector<TH1D*> projections3 = getCentPtProjections(filenames[i], histname3, averageNT, rebinFactor);
        std::vector<TH1D*> projections4 = getCentPtProjections(filenames[i], histname4, averageNT, rebinFactor);
        drawGraphs(gPad, projections1, projections2, projections3, projections4, color, markerStyle, averageNT, rebinFactor);
    }
  ...
  gPad->SetRightMargin(0.05); // 减小右侧边距
   TH2D *axisFrame1 = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0,5);
  ...
  axisFrame1->Draw("axis");  // 只绘制坐标轴，不绘制直方图
    for (int i = 0; i < 3; ++i) {
        ...
        std::vector<TH1D*> projections3 = getCentPtProjections(filenames[i], histname3, averageNT, rebinFactor);
        std::vector<TH1D*> projections4 = getCentPtProjections(filenames[i], histname4, averageNT, rebinFactor);
        drawGraphs1(gPad, projections1, projections2, projections3, projections4, color, markerStyle, averageNT, rebinFactor);
    }
     // 在纵坐标等于1处增加一条虚线
