...
void draw_ratio() {
    ...
    // 通用样式设置
    std::vector<Color_t> colors = {kRed, kBlue}; // 两种曲线颜色
    std::vector<Style_t> markerStyles = {21, 22}; // 两种标记样式
    
    // ============= Pad 1 =============
    c1->cd(1);
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame1 = new TH2D("axisFrame1", "", 100,-0.1, 2.99, 100, 0.002, 2.5);
        ...
        for (int i = 0; i < projections_toward.size(); i++) {
            // 计算特定RT区间与全RT区间的比率
            TH1D* ratio_hist = createRatioHistogram(projections_toward[i], hist_all_toward, 
                                                   Form("ratio_%d", i));
            ratio_hists.push_back(ratio_hist);
            TGraphErrors* graph_rt = drawConnectedPoints(ratio_hist, colors[i], markerStyles[i]);
        ...
        legend->AddEntry(drawConnectedPoints(ratio_hists[0], colors[0], markerStyles[0]), "0<R_{T}<0.5 / All R_{T}", "lp");
        legend->AddEntry(drawConnectedPoints(ratio_hists[1], colors[1], markerStyles[1]), "2.5<R_{T}<5 / All R_{T}", "lp");
        legend->Draw();
    ...
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame1 = new TH2D("axisFrame1", "", 100,-0.1, 2.99, 100, 0.002,6);
        axisFrame1->GetYaxis()->SetTickLength(0.02);
        axisFrame1->GetXaxis()->SetTickLength(0.02);
        axisFrame1->GetXaxis()->SetLabelSize(0.070);
        axisFrame1->GetYaxis()->SetLabelSize(0.070);
        ...
        axisFrame1->Draw("axis same");
        addText3(0.52, 0.90, "Transverse");
        addText4(0.80, 0.10, "#pi^{+}+#pi^{-}");
        addText2(0.92, 0.03, "(d)");
    }
    ...
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame2 = new TH2D("axisFrame2", "", 100, -0.1, 2.99, 100, 0.002, 2.5);
        ...
        label3->DrawLatexNDC(0.05, 0.5, "ratio to R_{T}>0");
    ...
    c1->cd(4);
    {
        ...
        TH2D *axisFrame2 = new TH2D("axisFrame2", "", 100, -0.1, 2.99, 100, 0.002, 6);
        axisFrame2->GetYaxis()->SetTickLength(0.02);
        axisFrame2->GetXaxis()->SetTickLength(0.02);
        axisFrame2->GetXaxis()->SetLabelSize(0.070);
        axisFrame2->GetYaxis()->SetLabelSize(0.070);
    ...
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame3 = new TH2D("axisFrame3", "", 100, -0.1, 2.99, 100, 0.01, 2.5);
    ...
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame3 = new TH2D("axisFrame3", "", 100, -0.1, 2.99, 100, 0.01, 6);
        axisFrame3->GetYaxis()->SetTickLength(0.02);
        axisFrame3->GetXaxis()->SetTickLength(0.02);
        axisFrame3->GetXaxis()->SetLabelSize(0.070);
        axisFrame3->GetYaxis()->SetLabelSize(0.070);
        ...
        addText4(0.80, 0.11, "p+bar{p}");
        addText2(0.92, 0.03, "(f)");
    }

    c1->SaveAs("pi_k_p_ratio.png");
    c1->SaveAs("pi_k_p_ratio.pdf");
}