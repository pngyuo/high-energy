...
TGraphErrors* analyzeAndDrawpi(const std::vector<TH1D*>& projections, TVirtualPad* pad, 
    ...
    for (const auto& h : projections) {
        ...
        // h->Draw();
        fitfun->SetParameters(0.2,200, 0.2, mass);
...
TGraphErrors* drawInJetRegion(const std::vector<TH1D*>& projectionsToward, 
    ...
    for (size_t j = 0; j < projectionsToward.size(); ++j) {
        ...
        fitfun->SetParameters(0.1,200, 0.3, mass);
...
void drawK() {
    ...
    TString filenames3[2]= {"proton_toward.root", "proton_transverse.root"};
        // 创建输出ROOT文件
        TFile *outFile = new TFile("k_results.root", "RECREATE");
    ...
    {
        gPad->SetTicks(1,1);
        TH2D *axisFrame1 = new TH2D("axisFrame1", "", 100, -0.1, 5.1, 100, 0.3, 2.2);
        ...
        for (int i = 0; i < 3; i++) {
            ...
            TFile* infile = new TFile(filenames2[0], "READ");
            if (infile && !infile->IsZombie()) {
                ...
                if (graph1) {
                    ...
                    graph1->Draw("P same");
                    graphsToward.push_back(graph1);  // 存储实验数据
                }
                delete infile;
            }
        }
    ...
    c1->cd(2);
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            TFile* infile = new TFile(filenames2[1], "READ");
            if (infile && !infile->IsZombie()) {
                TGraphErrors *graph1 = (TGraphErrors*)infile->Get("Pt");
                if (graph1) {
                    graph1->SetMarkerStyle(24);
                    ...
                    graph1->SetMarkerColor(kBlack);
                    graph1->Draw("P same");
                    graphsTransverse.push_back(graph1);
                }
                delete infile;
            }
        }
        addText2(0.35, 0.9, "Transverse");
        addText4(0.45, 0.25, "K^{+}+K^{-}");
    }
    ...
    c1->cd(3);
    {
        ...
        TH2D *axisFrame1 = new TH2D("axisFrame1", "", 50, -0.02, 1.15, 100,0.3, 2.2);
        axisFrame1->GetYaxis()->SetTickLength(0.02);
        axisFrame1->GetXaxis()->SetTickLength(0.02);
        axisFrame1->GetXaxis()->SetLabelSize(0.080);
        axisFrame1->GetYaxis()->SetLabelSize(0.080);
    ...
    outFile->Close();
    delete outFile;
    c1->SaveAs("k_test.png");
    c1->SaveAs("k_test.pdf");
}