...
std::vector<TH1D*> getCentPtProjections0(const TString& filename, const TString& histname) {
...
void drawP(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 1e-4, 1);
...
label->DrawLatexNDC(0.03, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
  ...
  TString histname2 = "hProton_dPhi1min"; 
  TString filenames1= "P.root";
    ...
    for (int i = 0; i < 3; i++) {
       ...
       std::vector<TH1D*> projections1 = getCentPtProjections0(filenames[i], histname1);
      std::vector<TH1D*> projections2 = getCentPtProjections1(filenames[i], histname2);
  ...
  c1->SaveAs("INEL P.png");
  //c1->SaveAs("INEL proton_.pdf");
}
