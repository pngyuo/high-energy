...
std::vector<TH1D*> getCentPtProjections(const TString& filename, const TString& histname) {
    ...
    histProjX->Rebin(1);  // 这里将每个2个bin合并为一个，你可以根据需要调整这个参数
...
void drawPiLog(){
  ...
  TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0, 0.35);
...
label->DrawLatexNDC(0.03, 0.5,  "(p+#bar{p})/(#pi^{+}+#pi^{-})"); // 使用NDC坐标

  TString filename[3] = {"hist_outputallFSI_liang.root","hist_outputnoFSI_liang.root","hist_outputnohFSI_liang.root"}; 
  TString histname1 = "hProton_dPhi0"; 
  TString histname2 = "hProton_dPhi1min";  
    TString histname3 = "hPiCh_dPhi0"; 
  TString histname4 = "hPiCh_dPhi1min"; 
