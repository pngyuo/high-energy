...
TGraphErrors* drawAveragePt(const std::vector<double>& avgPt, Color_t color, Style_t markerStyle) {
...
TGraphErrors* analyzeAndDraw(const std::vector<TH1D*>& projections, TVirtualPad* pad, Color_t color, Style_t markerStyle) {
    ...
    if (!avgPt.empty()) {
        return drawAveragePt(avgPt, color, markerStyle);
    }
...
void drawK() {
    ...
    TFile *infile1 = new TFile("K_To.root"); 
    TGraph *graph1 = (TGraph*)infile1->Get("RT");

    TFile *infile2 = new TFile("K_A.root");
    TGraph *graph2 = (TGraph*)infile2->Get("RT");

    TFile *infile3 = new TFile("K_Tr.root");
    TGraph *graph3 = (TGraph*)infile3->Get("RT");
    ...
    TH2D *axisFrame = new TH2D("axisFrame", "; ; ", 100, -0.1, 5.1, 100, 0.6, 1.3);
    ...
    if (j == 0) {
        ...
        graph3->SetMarkerColor(graph3->GetLineColor());
        graph3->Draw("p same");
    }
    ...
    c1->SaveAs("K_RT_PYTHIA8.png");
    c1->SaveAs("K_RT_PYTHIA8.pdf");
}