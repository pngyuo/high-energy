...
TGraphErrors* drawConnectedPointsWithErrors(TH1D* hist, Color_t color, Style_t markerStyle) {
...
void drawGraphs(const std::vector<TH1D*>& projections, Color_t color, Style_t markerStyle) {
  ...
  for (size_t i = 0; i < projections.size(); ++i) {
    ...
    TGraph* graph = drawConnectedPointsWithErrors(hist, color, markerStyle);
     if(i == 0) {
...
label->DrawLatexNDC(0.03, 0.5, "dN/(dp_{T}dy)(GeV/c)^{-1}"); // 使用NDC坐标
  ...
  TString filenames="hist_outputRope.root";
  ...
  std::vector<Style_t> markerStyles = {20, 21, 22, 23};
    ...
    legend->Draw(); 
  addText2(0.4, 0.8, "Rope");
  addText4(0.33, 0.25, "p+#bar{p}");
  c1->SetLogy(); // 设置y轴为对数刻度
  c1->SaveAs("proton_RoPt.png");
