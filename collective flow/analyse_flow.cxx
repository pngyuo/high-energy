...
int main(int argc, char** argv)
{
  ...
  TH1::SetDefaultSumw2();
	...
	TH2D *hDEtaDPhiSameEvent = new TH2D("hDEtaDPhiSameEvent", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
	TH2D *hDEtaDPhiMixEvent = new TH2D("hDEtaDPhiMixEvent", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);

	TH2D *hDEtaDPhiSameEventLowMid = new TH2D("hDEtaDPhiSameEventLowMid", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
	TH2D *hDEtaDPhiMixEventLowMid = new TH2D("hDEtaDPhiMixEventLowMid", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);

	TH2D *hDEtaDPhiSameEventHighMid = new TH2D("hDEtaDPhiSameEventHighMid", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
	TH2D *hDEtaDPhiMixEventHighMid = new TH2D("hDEtaDPhiMixEventHighMid", "; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
  ...
  //set 8 event classes, 9 Nsel bin edge
  const int N_cent=8;
  double Nsel_min=20;
  double Nsel_bin=20;
  ...
  for(int i=0; i<N_cent; i++) {
    ...
    hDEtaDPhiSameEvent_Cent[i] = new TH2D(Form("hDEtaDPhiSameEvent_Cent%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiMixEvent_Cent[i] = new TH2D(Form("hDEtaDPhiMixEvent_Cent%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
...
//    if(i==N_ptbin-1) ptbin_cut[N_ptbin]=ptbin_cut[i]+2*pt_bin;
    hDEtaDPhiSameEventHigh_ptbin[i] = new TH2D(Form("hDEtaDPhiSameEventHigh_ptbin%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiMixEventHigh_ptbin[i] = new TH2D(Form("hDEtaDPhiMixEventHigh_ptbin%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hTrigPtHigh_ptbin[i]= new TH1D(Form("hTrigPtHigh_ptbin%d",i), ";p_{T}", 500, 0, 10);
    hDEtaDPhiSameEventLow_ptbin[i] = new TH2D(Form("hDEtaDPhiSameEventLow_ptbin%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiMixEventLow_ptbin[i] = new TH2D(Form("hDEtaDPhiMixEventLow_ptbin%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hTrigPtLow_ptbin[i]= new TH1D(Form("hTrigPtLow_ptbin%d",i), ";p_{T}", 500, 0, 10);

    hDEtaDPhiSameEventHigh_ptbinTASame[i] = new TH2D(Form("hDEtaDPhiSameEventHigh_ptbinTASame%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiMixEventHigh_ptbinTASame[i] = new TH2D(Form("hDEtaDPhiMixEventHigh_ptbinTASame%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiSameEventLow_ptbinTASame[i] = new TH2D(Form("hDEtaDPhiSameEventLow_ptbinTASame%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
    hDEtaDPhiMixEventLow_ptbinTASame[i] = new TH2D(Form("hDEtaDPhiMixEventLow_ptbinTASame%d",i),"; #Delta#phi; #Delta#eta", 40, -0.5*PI, 1.5*PI, 200, -5, 5);
  ...
  PrintArray(ptbin_cut, N_ptbin+1);
	...
	TH3D *hNchDEtaDPhiSameEvent = new TH3D("hNchDEtaDPhiSameEvent", "; #Delta#phi; #Delta#eta; N_{trk}^{mid}", 40, -0.5*PI, 1.5*PI, 200, -5, 5, 300, -0.5, 299.5);
	TH3D *hNchDEtaDPhiMixEvent = new TH3D("hNchDEtaDPhiMixEvent", "; #Delta#phi; #Delta#eta; N_{trk}^{mid}", 40, -0.5*PI, 1.5*PI, 200, -5, 5, 300, -0.5, 299.5);
...
//  if(nEvents>8e6) nEvents=8e6;
  ...
  double accept_cut = 2.4;
	double highMultCut = 100;
	double lowMultCut = 20;
...
//			hSphericity->Fill(dMidCh, dSph);
    ...
    for(int i=0; i<N_cent; i++){
      ...
      //fill trig info 0~3
        ...
        //Nsel dependence
        if(icent>=0) hTrigPt_Cent[icent]->Fill(vec_trig.Pt());
      ...
      if(findPtBin>=N_ptbin||(findPtBin<0&&findPtBin!=-1)) {
        ...
        exit(1);
          ...
          //minbias or centrality dependence
            ...
            //Nsel dependence
            if(icent>=0) FillDPhiDEta(vec_trig, vec_tmp, hDEtaDPhiSameEvent_Cent[icent]);
